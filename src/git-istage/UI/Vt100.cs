...
internal static class Vt100
{
    ...
    extension(RenderBuffer buffer)
    {
        public void SetCursorPosition(int x, int y)
        {
            buffer.Write($"\e[{y + 1};{x + 1}H");
        }
        ...
        public void PositiveColors()
        {
            buffer.Write("\e[27m");
        }

        public void SetForegroundColor(TextColor? color = null)
        {
            if (color is null)
                buffer.Write("\e[39m");
            else
                buffer.WriteColor(color.Value, isForeground: true);
        }

        public void SetBackgroundColor(TextColor? color = null)
        {
            if (color is null)
                buffer.Write("\e[49m");
            else
                buffer.WriteColor(color.Value, isForeground: false);
        }

        public void EraseRestOfCurrentLine()
        {
            buffer.Write("\e[K");
        }

        private void WriteColor(TextColor color, bool isForeground)
        {
            var backgroundOffset = isForeground ? 0 : 10;
            var code = 30 + backgroundOffset;

            var (number, isBright) = color.AsVt100Color();
            if (number < 0)
            {
                code += 8;
                buffer.Write($"\e[{code};2;{color.R};{color.G};{color.B}m");
            }
            else
            {
                code += number;
                ...
                buffer.Write($"\e[{code}m");
            }
        }
    }

    extension(TextColor color)
    {
        public (int number, bool isBright) AsVt100Color()
        {
